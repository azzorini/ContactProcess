...
// Función que nos devuelve la correlación espacial de una simulación por debajo del
// landa crítico para la proyección en uno de los ejes
unsigned int get_spatial_correlation_length(const CPSim& sim, unsigned int ax = 0) {
        unsigned int N = sim.get_N(), N_data = sim.get_data_N();
        int pos_left, pos_right, max_length = 0;
	Position p;
		...
		for (unsigned int j = 0; j < N; j++) {
			pos_left = 0;
			if (ax == 0) p = Position(0, j);
			else p = Position(j, 0);
			while (pos_left < N && sim.get_data(i, p) == 0) {
				pos_left++; 
				if (ax == 0) p.i++;
				else p.j++;
			}
			...
			pos_right = N-1;
			if (ax == 0) p = Position(N-1, j);
			else p = Position(j, N-1);
			while (pos_right > pos_left && sim.get_data(i, p) == 0) {
				pos_right--;
				if (ax == 0) p.i--;
				else p.j--;
			}

			if (pos_right - pos_left + 1 > max_length) max_length = pos_right - pos_left + 1;
		}
        }
...
void simula_landas(int N, double max_tiempo, int start, int stop, const string& filename) {
	double tmax = max_tiempo, t_save, freq_t = 1;
	double landa;

	int i, j;
	ofstream ftime(("time_corr_lengths_" + filename).c_str()), fspace_x(("space_corr_lenghts_x_" + filename).c_str()), fspace_y(("space_corr_lenghts_y_" + filename).c_str());
	...
	for (i = start; i < stop; i++) {
		...
		ftime << landa; fspace_x << landa; fspace_y << landa;

		for (j = 0; j < 500; j++) {
			simulacion = CPSim(N, landa, 0, SEED*(i+10)*(j+1));
			...
			ftime << '\t' << simulacion.get_t();
			fspace_x << '\t' << get_spatial_correlation_length(simulacion, 0);
			fspace_y << '\t' << get_spatial_correlation_length(simulacion, 1);

		}
		ftime << '\n'; fspace_x << '\n'; fspace_y << '\n';
	}

	ftime.close(); fspace_x.close(); fspace_y.close();
}

void simula_landas_landa0(int N, double max_tiempo, int start, int stop, const string& filename) {
	double tmax = max_tiempo, t_save, freq_t = 1;
	double landa;
...
int main() {
	...
	//jthread t1(simula_landas, N, tmax, -4, 14, "2D_metodo1_grandes.txt");
	//jthread t2(simula_landas, N, tmax, 14, 21, "2D_metodo1_medianos.txt");
	//jthread t3(simula_landas, N, tmax, 21, 25, "2D_metodo1_pequeños.txt");

	//jthread t1(simula_landas, N, tmax, 25, 30, "2D_metodo1_mas_pequenyos.txt");
	//jthread t2(simula_landas, N, tmax, 30, 35, "2D_metodo1_extra_pequenyos.txt");
	
	simula_landas_landa0(N, tmax, 6, 10, "2D_metodo1_landa0");
}
