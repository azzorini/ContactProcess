...
unsigned int get_left_most(const CPSim& sim) {
...
unsigned int get_right_most(const CPSim& sim) {
	unsigned int i = sim.get_N() - 1;
	while (sim[i] == 0 && i > 0) i--;

	if (i == 0 && sim[i] == 0) return sim.get_N();
	else return i;
}

int main() {
	double tmax = 5000000, t_save = 0, freq_t = .5;
	int N = 1000;
	double landa;
	int i, n_random;
	CPSim simulacion;
	...
	for (int n = -18; n < -12; n++) {
		...
		while (i < 100) { 
			...
			simulacion = CPSim(N, landa, 0, SEED*n_random*(n+20));
			simulacion.activate(N/2);
			fleft.open("Pendientes/" + to_string(landa) + "_" + to_string(i) + "_L.txt");
			fright.open("Pendientes/" + to_string(landa) + "_" + to_string(i) + "_R.txt");

			while (simulacion.get_t() < tmax && simulacion.get_N_occ() != 0 && simulacion[0] == 0 && simulacion[N-1] == 0) {
				if (t_save < simulacion.get_t()) {
					fleft << simulacion.get_t() << '\t' << get_left_most(simulacion) << endl;
					fright << simulacion.get_t() << '\t' << get_right_most(simulacion) << endl;
					t_save += freq_t;
				}
				simulacion.update();
			}
			fleft.close(); fright.close();
			n_random++;
			if (simulacion[0] != 0 || simulacion[N-1] != 0) i++;
		}
	}
}
