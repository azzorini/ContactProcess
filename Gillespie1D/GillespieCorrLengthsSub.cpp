...
public:
	...
	// Constructores
	CPSim(int N = 0, double lambda = 1.0, int val = 0, unsigned int seed = SEED) : config(N, 0), landa(lambda), t(0), W(0), d(1), config_occ(), N_occ(0), times(), configs(), gen(seed), ran_u(DBL_TRUE_MIN, 1.0) {
		if (val != 0) {
			N_occ = W = N;
...
// landa crítico
unsigned int get_spatial_correlation_length(const CPSim& sim) {
	...
	for (unsigned int i = 0; i < N_data; i++) {
		pos_left = 0;
		while (sim.get_data(i, pos_left) == 0 && pos_left < N) pos_left++;

		pos_right = N-1;
		while (sim.get_data(i, pos_right) == 0 && pos_right > pos_left) pos_right--;
...
int main() {
	double tmax = 500000, t_save = 0, freq_t = .5;
	int N = 1000;
	double landa;
	CPSim simulacion;
	ofstream ftime("Gillespie_time_corr_lengths_1000_landa0.txt"), fspace("Gillespie_space_corr_lengths_1000_landa0.txt");
	
	for (int n = 1; n < 20; n++) {
		landa = pow(1.15, -n);
		cout << "Lanzando simulaciones para landa = " << landa << endl;
		ftime << landa; fspace << landa;
		for (unsigned int i = 0; i < 5000; i++) { 
			t_save = 0;

			simulacion = CPSim(N, landa, 0, SEED*(i+1)*(n+10));
			...
			fspace << '\t' << get_spatial_correlation_length(simulacion);
	//		cout << "Simulación número " << i << endl
	//		     << "Longitud de correlación temporal: " << simulacion.get_t() << endl
	//		     << "Longitud de correlación espacial: " << get_spatial_correlation_length(simulacion) << "\n\n";

	//		simulacion.save_to_pbm("simulations_lengths/sim_length_" + to_string(i) + ".pbm");
		}
