...
int main() {
	double tmax = 500, t_save = 0, freq_t = 1;
	int N = 100;
	double landa = 2.0;
	...
	while (simulacion.get_t() < tmax && simulacion.get_N_occ() != 0) {
		if (t_save < simulacion.get_t()) {
			...
			cout << simulacion.get_t() << '\t' << simulacion.get_rho() << endl;
			t_save += freq_t;
		}
		simulacion.update();
	}

	//simulacion.save_to_pbm("sim2d_2.4.pbm");
	simulacion.save_to_txt("sim2d_1.62.txt");
