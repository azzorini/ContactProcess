...
void simula_landas(int N, double tmax, int start, int stop, const string& filename) {
	...
	for (i = start; i < stop; i++) {
		landa = 3.29785 + pow(1.15, -i);
		{
			osyncstream out{cout};
			out << "[N = " << N << "] Lanzando simulaciones para landa = " << landa << '\n';
		...
		while (j < 10) {
			...
			while (simulacion.get_t() < tmax) {
				if (t_save < simulacion.get_t()) {
					if (simulacion.get_t() > 2000) {
...
int main() {
	...
	jthread t1(simula_landas, 100, tmax, 13, 19, "100_rho_vs_landa_medianos.txt");
	jthread t2(simula_landas, 100, tmax, 19, 24, "100_rho_vs_landa_pequenyos.txt");
}
