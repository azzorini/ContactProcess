...
void simula_landas(int N, double tmax, int start, int stop, const string& filename) {
	...
	int i, j;
	ofstream ftime(("time_corr_lengths_" + filename).c_str());
	...
	for (i = start; i < stop; i++) {
		...
		for (j = 0; j < 100; j++) {
			simulacion = CPSim(N, landa, 1, SEED*(i+10)*(j+1));
...
int main() {
	...
	jthread t1(simula_landas, N, tmax, -4, 18, "2D_metodo2_grandes.txt");
	jthread t2(simula_landas, N, tmax, 18, 29, "2D_metodo2_medianos.txt");
	jthread t3(simula_landas, N, tmax, 29, 35, "2D_metodo2_pequeños.txt");
}
