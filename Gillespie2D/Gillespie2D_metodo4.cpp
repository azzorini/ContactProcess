...
public:
	...
	int get_data(unsigned int t_i, const Position& p) const { return configs[t_i][p.i][p.j]; }
...
unsigned int get_left_most(const CPSim& sim, unsigned int ax = 0) {
	Position aux;
	unsigned int i = 0, N = sim.get_N();

	if (ax == 0) aux = Position(0, N/2);
	else aux = Position(N/2, 0);
...
unsigned int get_right_most(const CPSim& sim, unsigned int ax = 0) {
	Position aux;
	unsigned int i = sim.get_N() - 1;

	if (ax == 0) aux = Position(sim.get_N() - 1, sim.get_N()/2);
	else aux = Position(sim.get_N()/2, sim.get_N() - 1);
	...
	if (i == 0 && sim[aux] == 0) return 0;
	else return i;
}

bool toca_bordes(const CPSim& sim) {
	for (unsigned int i = 0; i < sim.get_N(); i++) {
		if (sim[Position(0, i)] != 0) return true;
		if (sim[Position(i, 0)] != 0) return true;
		if (sim[Position(sim.get_N() - 1, i)] != 0) return true;
		if (sim[Position(i, sim.get_N() - 1)] != 0) return true;
	}

	return false;
}

void simula_landas(int N, double max_tiempo, int start, int stop) {
	double tmax = max_tiempo, t_save, freq_t = 1;
	double landa;

	int i, j, n_random;
	ofstream fleft_x, fright_x, fleft_y, fright_y;
	...
	for (i = start; i < stop; i++) {
		...
		j = 0; n_random = 1;
		while (j < 50) {
			simulacion = CPSim(N, landa, 0, SEED*(i+20)*n_random);
			simulacion.activate(Position(N/2, N/2));
			fleft_x.open("Pendientes2D/" + to_string(landa) + "_" + to_string(2*j) + "_L.txt");
			fleft_y.open("Pendientes2D/" + to_string(landa) + "_" + to_string(2*j+1) + "_L.txt");
			fright_x.open("Pendientes2D/" + to_string(landa) + "_" + to_string(2*j) + "_R.txt");
			fright_y.open("Pendientes2D/" + to_string(landa) + "_" + to_string(2*j+1) + "_R.txt");
			t_save = .0;

			while (simulacion.get_t() < tmax && simulacion.get_N_occ() != 0 && !toca_bordes(simulacion)) {
				if (t_save < simulacion.get_t()) {
					fleft_x << simulacion.get_t() << '\t' << get_left_most(simulacion, 0) << '\n';
					fleft_y << simulacion.get_t() << '\t' << get_left_most(simulacion, 1) << '\n';
					fright_x << simulacion.get_t() << '\t' << get_right_most(simulacion, 0) << '\n';
					fright_y << simulacion.get_t() << '\t' << get_right_most(simulacion, 1) << '\n';
					t_save += freq_t;
				}
				simulacion.update();
			}
			fleft_x.close(); fleft_y.close(); fright_x.close(); fright_y.close();

			if (toca_bordes(simulacion)) j++;
			n_random++;
		}
	}
}
...
int main() {
	...
	jthread t1(simula_landas, N, tmax, -14, 8);
	jthread t2(simula_landas, N, tmax, 8, 18);
	jthread t3(simula_landas, N, tmax, 18, 25);
}
